...
parser.add_argument("-nt", "--number_of_threads",     type=int, default=1,  help="The number of threads")
...
#   deploy_command = 'wsk --apihost http://%s action update -i %s ./target/video.jar --main ch.ethz.systems.FFMPEG --docker ysun59/java8action -c %s'
#   deploy_command = 'wsk --apihost http://%s action update -i %s ./target/video.jar --main ch.ethz.systems.FFMPEG --docker openwhisk/java8action -c %s'
#   deploy_command = 'wsk --apihost http://%s action update -i %s ./target/video.jar --main ch.ethz.systems.FFMPEG --docker ysun59/java8action -c %s -m %s -t 300000'
    deploy_command = 'wsk --apihost http://%s action update -i %s ./target/video.jar --main ch.ethz.systems.FFMPEG --docker openwhisk/java8action -c %s -m %s -t 300000'
