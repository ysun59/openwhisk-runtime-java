...
parser.add_argument("-nt", "--number_of_threads",     type=int, default=10,  help="The number of threads")
...
parser.add_argument("-c", "--concurrency", type=int, default=3, help="The number of threads")
...
parser.add_argument("-m", "--memory", type=int, default=4000, help="Memory")
#parser.add_argument("-m", "--memory", type=int, default=800, help="Memory")
args = parser.parse_args()


def deploy_functions():
#   deploy_command = 'wsk --apihost http://%s action update -i %s ./target/image-classifier.jar --main ch.ethz.systems.InceptionImageClassifierDemo --docker openwhisk/java8action -c %s'
#   deploy_command = 'wsk --apihost http://%s action update -i %s ./target/image-classifier.jar --main ch.ethz.systems.InceptionImageClassifierDemo --docker ysun59/java8action -c %s -m %s -t 300000'
    deploy_command = 'wsk --apihost http://%s action update -i %s ./target/image-classifier.jar --main ch.ethz.systems.InceptionImageClassifierDemo --docker openwhisk/java8action -c %s -m %s -t 300000'
...
def main():
    ...
    def run_request(index):
        ...
        payload = { 'index' : random.randint(0,2) }
