...
#   deploy_command = 'wsk --apihost http://%s action update -i %s ./target/filehashing.jar --main ch.ethz.systems.FileHashing --docker ysun59/java8action -c %s'
    deploy_command = 'wsk --apihost http://%s action update -i %s ./target/filehashing.jar --main ch.ethz.systems.FileHashing --docker openwhisk/java8action -c %s'
...
#       payload = { 'time' : 1000 }
        payload = { 'seed' : random.randint(1, 100) }
